% time_perf.m  Per-step computational timing of the NMPC + EKF-SLAM loop.
...
cfg.sim_tim=20; cfg.tol=0.05;
mpc=mc_build_mpc(); L=size(cfg.lm,2); maxiter=round(cfg.sim_tim/mpc.T);

rng(2024);
t_solve=[]; t_ekf=[]; nTrials=10;
for tr=1:nTrials
  ...
  linit=true(1,L); P.dt=mpc.T; P.L=L; P.M=diag([cfg.var_v,cfg.var_w]); P.Q=diag([cfg.var_d,cfg.var_a]);
  x_true=cfg.x0_nom+nz.offset; args=mpc.args; u0=zeros(mpc.N,mpc.n_controls); k=0;
  while k<maxiter
    x_fb=X(1:3);
    if norm(x_fb(1:2)-cfg.xs(1:2))<cfg.tol, break; end
    args.p=[x_fb;cfg.xs]; args.x0=reshape(u0',mpc.n_controls*mpc.N,1);
    ts=tic;
    sol=mpc.solver('x0',args.x0,'lbx',args.lbx,'ubx',args.ubx,'lbg',args.lbg,'ubg',args.ubg,'p',args.p);
    t_solve(end+1)=toc(ts)*1000;
    u=reshape(full(sol.x)',mpc.n_controls,mpc.N)'; u_cmd=u(1,:)';
    u_act=u_cmd+nz.u(:,k+1);
    v=u_act(1); w=u_act(2); if abs(w)<1e-9, w=1e-9; end; th=x_true(3);
    x_true=x_true+[v/w*(sin(th+w*mpc.T)-sin(th)); v/w*(cos(th)-cos(th+w*mpc.T)); w*mpc.T]; x_true(3)=wrapToPi(x_true(3));
    z=zeros(2,L); for i=1:L, dx=cfg.lm(1,i)-x_true(1); dy=cfg.lm(2,i)-x_true(2); z(:,i)=[sqrt(dx^2+dy^2);wrapToPi(atan2(dy,dx)-x_true(3))]; end
    z=z+nz.z(:,:,k+1);
    te=tic;
    [X,Sigma,linit]=mc_ekf_step(X,Sigma,u_cmd,z,linit,P,true);
    t_ekf(end+1)=toc(te)*1000;
    u0=[u(2:end,:);u(end,:)]; k=k+1;
  end
end

T_ms=mpc.T*1000;
p95=@(v) q_pct(v,95);
fprintf('\n=== Per-step timing: %d steps over %d trials, T=%.0f ms ===\n',numel(t_solve),nTrials,T_ms);
