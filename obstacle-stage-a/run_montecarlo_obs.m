...
cfg.cov_aware = false;          % Stage A (deterministic). Flip to true for Stage B.
cfg.gamma     = 2.0;            % chance-constraint factor (used only if cov_aware)
...
save('mc_results_obs.mat','results','cfg','obstacle','M');
fprintf('Saved mc_results_obs.mat\n');
...
title('Obstacle avoidance: true trajectories (trial 1)');
set(gca,'FontName','Times New Roman','FontSize',12);
saveas(f1,'mc_obs_trajectories.png');
...
ylabel('Collision rate (%)'); title('Collisions by feedback source'); grid on
saveas(f2,'mc_obs_collision.png');

fprintf('Saved mc_obs_trajectories.png and mc_obs_collision.png\n');
