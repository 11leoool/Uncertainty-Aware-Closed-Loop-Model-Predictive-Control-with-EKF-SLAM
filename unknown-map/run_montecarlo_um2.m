% RUN_MONTECARLO_UM2  Stage 1b of the UNKNOWN-MAP refactor: multi-waypoint
...
lines = {};
lines{end+1} = sprintf('=== Unknown-map TOUR Monte-Carlo (M = %d, ~8.4 m, 3 waypoints) ===', M);
...
disp(txt);
fid = fopen('mc_um2_results.txt','w'); fprintf(fid,'%s\n',txt); fclose(fid);

save('mc_um2_results.mat','results','cfg','M');
fprintf('Saved mc_um2_results.mat / .txt\n');
...
saveas(f1,'mc_um2_terminal_error.png');
...
saveas(f2,'mc_um2_trajectories.png');
...
set(gca,'FontName','Times New Roman','FontSize',12);
saveas(f3,'mc_um2_sigma.png');

fprintf('Saved mc_um2_terminal_error.png, mc_um2_trajectories.png, mc_um2_sigma.png\n');
