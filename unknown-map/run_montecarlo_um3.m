% RUN_MONTECARLO_UM3  Stage 1c: as Stage 1b PLUS a finite sensing range -- a landmark is UNKNOWN until the robot first comes within cfg.r_sense of it (incremental mapping). Based on of the UNKNOWN-MAP refactor: multi-waypoint
...
cfg.lm   = [-0.5 1 -1.5;
             0.5 1  0.0];
cfg.wps  = [ 1.5  -1.5   1.2;          % waypoint x (visited in order)
             1.5   1.0  -0.8];         % waypoint y ; last = final goal
cfg.x0_nom = [0; 0; 0];
...
cfg.r_sense = 1.5;              % landmark sensing range (m): unknown until detected
...
lines = {};
lines{end+1} = sprintf('=== Unknown-map TOUR + sensing range (M = %d, r_sense = 1.5 m) ===', M);
...
disp(txt);
fid = fopen('mc_um3_results.txt','w'); fprintf(fid,'%s\n',txt); fclose(fid);

save('mc_um3_results.mat','results','cfg','M');
fprintf('Saved mc_um3_results.mat / .txt\n');
...
saveas(f1,'mc_um3_terminal_error.png');
...
saveas(f2,'mc_um3_trajectories.png');
...
set(gca,'FontName','Times New Roman','FontSize',12);
saveas(f3,'mc_um3_sigma.png');

fprintf('Saved mc_um3_terminal_error.png, mc_um3_trajectories.png, mc_um3_sigma.png\n');
