...
collided = false;
infl_sum = 0; infl_n = 0;          % accumulate applied inflation (delta - base)
...
while k < maxiter
    ...
    infl_sum = infl_sum + (delta - base_margin); infl_n = infl_n + 1;
...
res.path_len      = sum(sqrt(sum(diff(traj_true(1:2,1:k+1),1,2).^2,1)));
